...
import { Star, TrendingUp, Play, Info, ChevronDown } from 'lucide-react'
...
const Movies = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null)
      try {
  const { data } = await axios.get('/api/content', { params: { type: 'MOVIE', limit: 60 } })
  ...
  const getEmotionColor = (emotion) => {
    const colors = {
      joy: 'bg-yellow-400 text-yellow-900',
      excitement: 'bg-orange-400 text-orange-900',
      tension: 'bg-red-400 text-red-900',
      awe: 'bg-purple-400 text-purple-900',
  ...
  const filteredAndSortedMovies = items
    .filter(movie => filterGenre === 'all' || (movie.genre || '').trim() === filterGenre)
    .sort((a, b) => {
      switch (sortBy) {
        case 'rating': return (Number(b.avg_rating) || 0) - (Number(a.avg_rating) || 0)
        case 'hype': return (Number(b.hype_index) || 0) - (Number(a.hype_index) || 0)
...
const MovieCard = ({ movie, isHovered, onHover, onLeave }) => {
  ...
  useEffect(() => {
    if (videoRef.current) {
      if (isHovered && movie.trailer_url) {
  ...
  const posterUrl = isValidUrl(movie.poster_url)
    ? movie.poster_url
    : 'https://placehold.co/400x600/1e293b/ffffff?text=No+Poster'
  const trailerUrl = isValidUrl(movie.trailer_url) ? movie.trailer_url : null

  // Normalize people photos
  const directorPhoto = isValidUrl(movie.director_photo_url) ? movie.director_photo_url : null
  const castPhotos = (() => {
    const src = movie.cast_photos
    if (!src) return []
    ...
    <Link to={`/movie/${movie.id}`}>
      ...
      >
        ...
        <div className="relative aspect-[2/3] overflow-hidden">
          ...
          {/* Градиент-оверлей с улучшенным эффектом */}
          ...
          {/* Рейтинги (верхний правый угол) - улучшенный дизайн */}
          <div className="absolute top-4 right-4 flex flex-col gap-2.5">
            {movie.avg_rating > 0 && (
              <div className="bg-gradient-to-r from-accent-500 to-accent-600 backdrop-blur-md px-3.5 py-2 rounded-xl flex items-center gap-2 shadow-lg shadow-accent-500/40 transform group-hover:scale-110 transition-transform duration-300">
                ...
                <span className="font-bold text-sm">{Number(movie.avg_rating).toFixed(1)}</span>
              </div>
            )}
            {movie.hype_index > 0 && (
              <div className="bg-gradient-to-r from-red-500 to-red-600 backdrop-blur-md px-3.5 py-2 rounded-xl flex items-center gap-2 shadow-lg shadow-red-500/40 transform group-hover:scale-110 transition-transform duration-300">
                ...
                <span className="font-bold text-sm">{movie.hype_index}%</span>
              </div>
            )}
          </div>

          {/* Кнопка воспроизведения при наведении - улучшенный эффект */}
          {trailerUrl && (
