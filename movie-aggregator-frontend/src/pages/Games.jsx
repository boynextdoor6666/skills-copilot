...
import { Star, TrendingUp, Play, Info, Gamepad2, ChevronDown } from 'lucide-react'
...
const Games = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null)
      try {
  const { data } = await axios.get('/api/content', { params: { type: 'GAME', limit: 60 } })
        if (!cancelled) setItems(Array.isArray(data) ? data : [])
      } catch (e) {
        if (!cancelled) setError('Не удалось загрузить игры')
      } finally {
  ...
  const filteredGames = items
    .filter(g => filterGenre === 'all' || (g.genre || '').trim() === filterGenre)
    .sort((a, b) => {
...
const GameCard = ({ game, isHovered, onHover, onLeave }) => {
  ...
  const posterUrl = isValidUrl(game.poster_url)
    ? game.poster_url
    : 'https://placehold.co/400x600/1e293b/ffffff?text=No+Poster'
  const trailerUrl = isValidUrl(game.trailer_url) ? game.trailer_url : null

  // Normalize people photos (for game, may be director/lead designer etc. if provided)
  const directorPhoto = isValidUrl(game.director_photo_url) ? game.director_photo_url : null
  const castPhotos = (() => {
    const src = game.cast_photos
    if (!src) return []
          ...
          <div className="absolute top-4 right-4 flex flex-col gap-2.5">
            {game.avg_rating > 0 && (
              <div className="bg-gradient-to-r from-accent-500 to-accent-600 backdrop-blur-md px-3.5 py-2 rounded-xl flex items-center gap-2 shadow-lg shadow-accent-500/40 transform group-hover:scale-110 transition-transform duration-300">
                ...
                <span className="font-bold text-sm">{Number(game.avg_rating).toFixed(1)}</span>
              </div>
            )}
            {game.hype_index > 0 && (
              <div className="bg-gradient-to-r from-red-500 to-red-600 backdrop-blur-md px-3.5 py-2 rounded-xl flex items-center gap-2 shadow-lg shadow-red-500/40 transform group-hover:scale-110 transition-transform duration-300">
                ...
                <span className="font-bold text-sm">{game.hype_index}%</span>
              </div>
            )}
          </div>
